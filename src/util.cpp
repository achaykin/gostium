...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gostium";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "gostium.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "gostiumd.pid"));
