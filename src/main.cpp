...
uint256 hashGenesisBlock("0x00000088d9df548cf7ca45a2d9774c427d4eb5823871463a28d43cf8ac3c9a13");
...
const string strMessageMagic = "gostium Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight < 306600) {
        nSubsidy = 100 * COIN;
    } else if (nHeight < 613200) { // All luck is seven ;)
        nSubsidy = 50 * COIN;
    } else if (nHeight == 766500) {
        nSubsidy = 10 * COIN;
    } else {
        nSubsidy >>= (nHeight / 919800); // Gostium: 919800 blocks in ~6 years
    }
...
static const int64 nTargetTimespan = 86184; //420 * 205.2; = 86184 // Gostium: 420 blocks
static const int64 nTargetSpacing = 205;//3.42 * 60; // Gostium: 3.42 minutes
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    /* current difficulty formula, Gostium - kimoto gravity well */
...
unsigned int static OldGetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gostium difficulty adjustment protocol switch (Thanks to FeatherCoin for this idea)
    ...
    // Gostium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00006b1b0b8779145f5ca5cf9c9396947ee77aa88d30d6354ffe35f46fdc3ca");
    }
...
///////////////////////  
// Genesis block
        const char* pszTimestamp = fTestNet ? "28/08/2017 GOSTIUM COIN LAUNCH" : "10/08/2017:  GOSTD ALGO CREATED";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////// 
		...
		if (fTestNet)
			assert(block.hashMerkleRoot == uint256("0x8b24d6b42c09abb3e3590805b17d057b00257fde21c6b6e7ffb2fb770dc1b2b1"));
		else	
        	assert(block.hashMerkleRoot == uint256("0x4b7d222214da450ae0582ffde3d96fd86f437640801db40e9e8499f9d0a08ab3"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GostiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GostiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GostiumMiner(CWallet *pwallet)
{
    printf("GostiumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gostium-miner");
	...
	{ 
		loop 
		{
		   ...
		   while (vNodes.empty())
		    ...
		    printf("Running GostiumMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GostiumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
   ...
   /* if (nThreads < 0)
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GostiumMiner, pwallet));
}
