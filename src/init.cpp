    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("GOSTcoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  gostiumd [options]                     " + "\n" +
                  "  gostiumd [options] <command> [params]  " + _("Send command to -server or gostiumd") + "\n" +
                  "  gostiumd [options] help                " + _("List commands") + "\n" +
                  "  gostiumd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "gostium:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: gostium.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: gostiumd.pid)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Gostium Wiki for SSL setup instructions)") + "\n" +
        ...
        "  -i2psessionname=<session name>         " + _("Name of an I2P session. If it is not specified, value will be \"Gostium-client\"") + "\n" +
...
//	}
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Gostium is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Gostium version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Gostium server starting\n");

    if (!GetBoolArg("-stfu", false) && !IsI2POnly()) {
	InitWarning("Gostium is running on clearnet!\n");
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Gostium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Gostium to complete") << "\n";
